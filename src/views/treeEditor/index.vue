 ...
 * @Date: 2022-08-17 09:54:41
 * @LastEditors: zhangxi
 * @LastEditTime: 2022-08-26 17:01:31
 * @FilePath: \vue3-vite-admin\src\views\treeEditor\index.vue
 * @Description: 树形图表编辑器
-->
<template>
  <div class="main-box">
...
export default {
  ...
  methods: {
    setNodeInfo(tree) {
      ...
      const treeGenerator = D3.tree().nodeSize([50, 180]);
      const treeData = treeGenerator(data);
      // 获取自动布局后的节点信息
      const nodes = treeData.descendants();
      console.log(nodes, "-----------nodes");
      const beginPostion = this.getBeginPostion(nodes);
      console.log(beginPostion, "---------------bepostion");

      // 获取父子关系列表
      const links = treeData.links();
      // 设置节点位置信息，水平排列x、y轴位置互换原理
      this.nodeList = nodes.map((item) => {
        return {
          id: item.data.id,
          name: item.data.name,
          left: `${item.y + 20}px`, // 左方向起始位置
          top: `${item.x + beginPostion + 20}px`,
        };
      });
